...
import { readableStreamToString, SesameHrError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SesameHrError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SesameHrError(e, e.response?.data)
                }
                if (innerError instanceof SesameHrError) {
                    // Got "SesameHrError" from the above try block
                    throw innerError;
                }
